...
import {LoraCard,type AdapterValue} from './lora-card';
...
import {originalExtension,sourceLabel,runProgress,activeStates,cloneDraft,configChanges,createBatch,defaultAdapters,defaultConfig,draftForCreationMode,formatDate,formatTime,initialDraft,makeSamples,recoverState,safeFilename,stageLabels,terminalStates,trackFromRun,transitionRun,uid,validateDraft,type Config,type CreationMode,type Draft,type Run,type SavedState,type Stage,type Track} from './domain';
...
export function App(){
  ...
  const clearAbc=()=>{
    const before=draft.abc??'';
    if(!before.trim())return;
    updateDraft({abc:''});
    advancedTranscriptionRequest.current++;
    setAdvancedTranscriptionState({status:'idle'});
    toast('已清空 ABC 乐谱。',()=>updateDraft({abc:before}),'撤销');
    requestAnimationFrame(()=>abcFieldRef.current?.querySelector('textarea')?.focus());
  };
  const clearLyrics=()=>{
    const before=draft.lyrics;
    if(!before.trim())return;
    updateDraft({lyrics:''});
    toast('已清空歌词。',()=>updateDraft({lyrics:before}),'撤销');
    requestAnimationFrame(()=>lyricsRef.current?.focus());
  };
  const clearStyle=()=>{
    const before=draft.style;
    if(!before.trim())return;
    updateDraft({style:''});
    toast('已清空风格描述。',()=>updateDraft({style:before}),'撤销');
    requestAnimationFrame(()=>styleRef.current?.focus());
  };
  ...
  return <ThemeContext.Provider value={theme}><MotionConfig reducedMotion={reduced?'always':'user'} transition={reduced?{duration:0}:motionTransition}><div data-module={module} onWheelCapture={handleWorkspaceWheel} className={`studio-app ${solid?'solid-surfaces':''} ${reduced?'reduced-motion':''}`}>
    ...
    <div className="workspace-shell"><nav className="workspace-navigation" aria-label="工作模块"><button aria-current={module==='create'?'page':undefined} onClick={()=>setModule('create')}>{module==='create'?<motion.span className="nav-pill" layoutId="module-pill" transition={{duration:.22,ease:[.22,1,.36,1]}} aria-hidden="true"/>:null}<WaveformIcon size={23}/>创作</button><button aria-current={module==='score'?'page':undefined} onClick={()=>setModule('score')}>{module==='score'?<motion.span className="nav-pill" layoutId="module-pill" transition={{duration:.22,ease:[.22,1,.36,1]}} aria-hidden="true"/>:null}<MusicNotesIcon size={23}/>谱曲{scoreActive>0&&<span>{scoreActive}</span>}</button></nav><div className="workspace-pages"><main className="workbench" data-panel={mobilePanel} data-active={module==='create'} inert={module!=='create'} aria-hidden={module!=='create'}>
      <section className="composer glass" aria-label="创作区"><div className="creation-tabs" role="tablist" aria-label="创作模式">{([['quick','快速创作'],['advanced','高级创作']] as const).map(([value,label])=><button key={value} role="tab" aria-selected={creationMode===value} onClick={()=>setCreationMode(value)}>{creationMode===value?<motion.span className="tab-pill" layoutId="creation-mode-pill" transition={{duration:.2,ease:[.22,1,.36,1]}} aria-hidden="true"/>:null}{label}</button>)}</div><div className="composer-scroll"><div className="composer-topline"><span>创作</span><span className={`save-status ${saveState==='failed'?'error':''}`}>{saveState==='saving'?'保存中…':saveState==='failed'?'草稿未保存':<><CheckIcon size={13}/>草稿已保存</>}</span><Menu.Root><Menu.Trigger asChild><IconButton label="创作选项"><MoreIcon/></IconButton></Menu.Trigger><Menu.Portal><Menu.Content className="dropdown" align="end"><Menu.Item onSelect={clearDraft}><PencilIcon/>新建创作</Menu.Item><Menu.Item onSelect={()=>{const url=URL.createObjectURL(new Blob([draft.lyrics],{type:'text/plain;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=safeFilename(draft.title||'歌词')+'.txt';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}}><DownloadIcon/>保存歌词文本</Menu.Item></Menu.Content></Menu.Portal></Menu.Root></div>
        ...
        <LoraCard active={module==='create'} value={draft.config.adapters??defaultAdapters} onChange={(adapters:AdapterValue)=>updateDraft({config:{...draft.config,adapters}})}/>
        ...
        <section ref={abcFieldRef} inert={creationMode!=='advanced'} aria-hidden={creationMode!=='advanced'} data-collapsed={creationMode!=='advanced'} className="abc-field creative-card field" aria-labelledby="abc-heading"><div className="creative-heading field-heading"><CodeIcon size={19}/><label id="abc-heading" htmlFor="field-abc">ABC 乐谱 <small className="abc-optional">可选</small></label><button type="button" className="text-button abc-clear" disabled={!draft.abc?.trim()} onClick={clearAbc}>清空</button><button type="button" onClick={()=>abcInput.current?.click()}>导入 .abc</button></div><textarea id="field-abc" value={draft.abc??''} onChange={e=>updateDraft({abc:e.target.value})} placeholder={'粘贴或导入 ABC 乐谱…\nX:1\nM:4/4\nK:C'}/><p className="arrangement-help">可选。填入后按 ABC 中的旋律与和声生成；留空则按歌词与风格自动规划。</p><button className="style-chat-trigger" aria-label="AI 辅助编辑 ABC 乐谱" onClick={()=>openChat('abc-chat')}><SparkleIcon size={22}/></button></section>
<div className={`lyrics-field field ${theme==='dark'&&lyricsCollapsed?'field-collapsed':''}`}><div className="field-heading">{theme==='dark'?<><button className="field-collapse" aria-expanded={!lyricsCollapsed} aria-controls="lyrics-input-region" onClick={()=>setLyricsCollapsed(v=>!v)}><CaretDownIcon className={lyricsCollapsed?'collapsed-arrow':''}/>歌词</button><label className="sr-only" htmlFor="field-lyrics">歌词</label></>:<label htmlFor="field-lyrics">歌词</label>}<button type="button" className="text-button field-clear" disabled={!draft.lyrics.trim()} onClick={clearLyrics}>清空</button></div><div id="lyrics-input-region" inert={theme==='dark'&&lyricsCollapsed} aria-hidden={theme==='dark'&&lyricsCollapsed} data-collapsed={theme==='dark'&&lyricsCollapsed} className={`textarea-shell ${errors.some(e=>e.field==='lyrics')?'invalid':''}`}><textarea ref={lyricsRef} id="field-lyrics" value={draft.lyrics} onChange={e=>updateDraft({lyrics:e.target.value})} spellCheck={false} aria-invalid={errors.some(e=>e.field==='lyrics')} aria-describedby="lyrics-error" placeholder={'留空生成纯音乐，或写下你想唱的话…\n[Verse]'}/><span className="style-count">{Array.from(draft.lyrics).length} 字</span></div><p id="lyrics-error" className="field-error">{errors.find(e=>e.field==='lyrics')?.message}</p><button className="style-chat-trigger" aria-label="AI 辅助编辑歌词" onClick={()=>openChat('lyrics-chat')}><SparkleIcon size={22}/></button></div>
<div className={`style-field field ${theme==='dark'&&styleCollapsed?'field-collapsed':''}`}><div className="field-heading">{theme==='dark'?<><button className="field-collapse" aria-expanded={!styleCollapsed} aria-controls="style-input-region" onClick={()=>setStyleCollapsed(v=>!v)}><CaretDownIcon className={styleCollapsed?'collapsed-arrow':''}/>风格描述</button><label className="sr-only" htmlFor="field-style">风格描述</label></>:<label htmlFor="field-style">风格描述</label>}<button type="button" className="text-button field-clear" disabled={!draft.style.trim()} onClick={clearStyle}>清空</button></div><div id="style-input-region" inert={theme==='dark'&&styleCollapsed} aria-hidden={theme==='dark'&&styleCollapsed} data-collapsed={theme==='dark'&&styleCollapsed} className={`textarea-shell ${errors.some(e=>e.field==='style')?'invalid':''}`}><textarea ref={styleRef} id="field-style" value={draft.style} onChange={e=>updateDraft({style:e.target.value})} aria-invalid={errors.some(e=>e.field==='style')} placeholder="曲风、人声、乐器与氛围…"/><span className="style-count">{Array.from(draft.style).length} 字</span></div>{errors.some(e=>e.field==='style')?<p className="field-error">{errors.find(e=>e.field==='style')?.message}</p>:null}<EditorResizeHandle label="风格描述"/><button className="style-chat-trigger" aria-label="AI 辅助优化风格" onClick={()=>openChat('style-chat')}><SparkleIcon size={22}/></button></div>
