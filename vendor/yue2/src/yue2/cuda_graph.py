...
class GraphAR:
    ...
    def __init__(self, model, prefixes, max_tokens, *, capture=True, attention_backend="auto", fuse_projections=False):
        ...
        fp8_active = bool(getattr(model, "_yue2_fp8_originals", {}))
        if fp8_active and not getattr(model, "_yue2_fp8_graph_validated", False):
            raise ValueError("FP8 GraphAR requires runtime validation")
        if fp8_active and fuse_projections:
            raise ValueError("FP8 CUDA Graph does not support projection fusion")
        if any(not ((isinstance(module, torch.nn.Linear) and module.weight.dtype == self.dtype)
                    or (fp8_active and isinstance(module, FP8Linear)
                        and module.weight.dtype == torch.float8_e4m3fn)) for module in linears):
            raise ValueError("GraphAR requires standard Linear or validated FP8Linear")
    ...
    def _capture(self):
        with torch.cuda.device(self.device):
            ...
            # FP8 GEMM workspaces are cached per stream by cuBLAS. Reuse the
            # warmup stream across phases to avoid one persistent workspace per song.
            warmup = getattr(self.model, "_yue2_fp8_warmup_stream", None) if getattr(self.model, "_yue2_fp8_originals", {}) else None
            if warmup is None:
                warmup = torch.cuda.Stream(device=self.device)
                if getattr(self.model, "_yue2_fp8_originals", {}):
                    self.model._yue2_fp8_warmup_stream = warmup
