...
class FP8Linear(nn.Module):
    ...
    def forward(self, value):
        ...
        # GraphAR batches CFG branches; eager evaluates them independently.
        # Preserve the per-branch activation scale instead of mixing branches.
        if value.ndim == 3 and value.shape[0] > 1:
            return torch.cat([self.forward(branch) for branch in value.split(1, dim=0)], dim=0)
        shape = value.shape
